...
pygame.display.set_caption("The Chosen - Story Mode") # Typo "THe" diperbaiki

# Colors
RED = (255, 0, 0); YELLOW = (255, 255, 0); WHITE = (255, 255, 255)
GREY = (200, 200, 200); BLACK = (0, 0, 0); GREEN = (0, 180, 0)
DARK_RED = (180, 0, 0)
DIALOG_BOX_COLOR = (20, 20, 60, 220) # Biru tua, semi-transparan untuk dialog box
...
clock = pygame.time.Clock() # Clock untuk timing
...
font_dialog_text = pygame.font.Font("assets/fonts/turok.ttf", 24) # Font untuk teks dialog box
font_speaker_name = pygame.font.Font("assets/fonts/turok.ttf", 26) # Font untuk nama pembicara
...
story_opponents = {
    ...
    "golem": Mobs(2, 700, SCREEN_HEIGHT, True, # Diubah dari skeleton ke golem untuk Bab 2
    ...
    "assasin": Warrior(2, 700, SCREEN_HEIGHT, True, # Assassin menggunakan kelas Warrior
    ...
    "monk": Warrior(2, 700, SCREEN_HEIGHT, True, # Monk (Stevanus) menggunakan kelas Warrior
...
lore_chapters_data = {
    ...
    4: {
        ...
        "opponent_id": "monk", # Key untuk story_opponents
        "dialogue_sequence": [
...
TEXT_AREA_RECT = pygame.Rect(
    ...
    DIALOG_BOX_RECT.width - TEXT_AREA_MARGIN_X - 20, # -20 for right margin
    DIALOG_BOX_RECT.height - (TEXT_AREA_MARGIN_Y * 2)
)
FACE_POS = (DIALOG_BOX_RECT.x + 15, DIALOG_BOX_RECT.y + (DIALOG_BOX_RECT.height - 110) // 2) # Centered vertically a bit
FACE_SIZE = (110, 110)
...
def show_dialogue_box_enhanced(current_screen_content_func, text_segment, speaker_id_str, speaker_face_surf):
    try:
        if not pygame.mixer.music.get_busy() or pygame.mixer.music.get_pos() == -1:
            pygame.mixer.music.load("assets/audio/story_dialogue.wav") # Ganti dengan musik dialog yang sesuai
    ...
    while waiting_for_page_input:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    return False # Player skip
                current_page_index += 1
                if current_page_index >= num_pages:
                    waiting_for_page_input = False # All pages for this segment shown

        current_screen_content_func() # Gambar latar belakang (misal, draw_bg_story)
        ...
        pygame.draw.rect(screen, DIALOG_BORDER_COLOR, DIALOG_BOX_RECT, 3) # Border
        ...
        screen.blit(name_surf, (TEXT_AREA_RECT.x, DIALOG_BOX_RECT.y + 5)) # Di atas area teks
...
def _play_dialogue_sequence_enhanced(current_screen_func, dialog_list):
    # Panggil current_screen_func sekali di awal sequence jika perlu memastikan latar tergambar
    current_screen_func()
    pygame.display.flip()

    for segment_info in dialog_list:
        speaker_key = segment_info["speaker"] # "warrior", "assasin", "monk", "narrator"
        ...
        if not show_dialogue_box_enhanced(current_screen_func, text_for_segment, speaker_key, char_face_surface):
            return False # Player skipped
    return True

# --- Helper functions for battle module callback ---
def draw_bg_story_cb(): screen.blit(pygame.transform.scale(bg_image, (SCREEN_WIDTH, SCREEN_HEIGHT)), (0, 0))
...
def start_story_battle_flow(player, opponent_instance):
    ...
    # Hentikan musik dialog sebelum pertempuran
    ...
    # Setelah pertempuran, musik dialog mungkin perlu dimulai lagi oleh _play_dialogue_sequence_enhanced jika ada dialog outro
    return return_signal

# --- Main Story Function ---
def main_story(start_new_game_flag=False):
    global player_fighter_story
    ...
    if game_state.get("story_completed", False) and not start_new_game_flag:
        draw_bg_story_cb() # Gambar latar sebelum dialog "story completed"
    ...
    while running_story_mode and current_chapter_num <= MAX_CHAPTERS:
        ...
        if not chapter_data: print(f"Error: Chapter {current_chapter_num} data missing."); break
        ...
        draw_bg_story_cb() # Gambar latar sebelum dialog intro
        ...
        draw_bg_story_cb() # Gambar latar sebelum dialog outro
        if not _play_dialogue_sequence_enhanced(draw_bg_story_cb, chapter_data[outro_dialogue_key]): return True

        if player_fighter_story.alive: # Player WON
            if current_chapter_num == 4: # Defeated Monk
                ...
                running_story_mode = False # End story mode successfully
            ...
            # Dialog "Retry?" akan ditangani oleh _play_dialogue_sequence_enhanced jika dimasukkan ke outro_lose
            # Untuk sekarang, kita anggap player harus mengulang dari menu utama jika kalah atau implementasi retry terpisah.
            # Untuk kesederhanaan, jika kalah, pemain kembali ke menu.
            # (Atau, bisa tambahkan opsi retry di sini setelah dialog kalah)
            # if not _play_dialogue_sequence_enhanced(draw_bg_story_cb, [{"speaker": "narrator", "lines": "Retry this chapter? (Any key / ESC)"}]):
            # return True # Player memilih tidak retry
            pass # Loop akan mencoba chapter yang sama lagi jika tidak ada return True
    ...
    elif not running_story_mode and current_chapter_num <=MAX_CHAPTERS: # Loop dihentikan karena error, bukan selesai
    ...
    if pygame.mixer.music.get_busy(): pygame.mixer.music.stop() # Pastikan musik dialog berhenti
    return True # Kembali ke main menu

if __name__ == "__main__":
    if not os.path.exists(SAVE_FILE_NAME): # Gunakan SAVE_FILE_NAME dari main.py atau definisikan di sini
